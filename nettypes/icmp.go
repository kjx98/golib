...
func (p ICMP_P) String(frameLen uint16, indent int) string {
	...
	s := fmt.Sprintf(padLeft("ICMP Len     : %d\n", "\t", indent), frameLen) +
		...
		fmt.Sprintf(padLeft("CalcChecksum : %02x\n", "\t", indent), HToNS(p.CalculateChecksum(frameLen)))
	if len(ps) > 0 {
...
func (p ICMP_P) CalculateChecksum(frameLen uint16) uint16 {
	cs := uint32(byteOrder.Uint16(p[0:2])) +
		uint32(byteOrder.Uint16(p[4:6])) +
		uint32(byteOrder.Uint16(p[6:8]))
	frameLen -= 8
	i := 8
	for ; frameLen > 1; i, frameLen = i+2, frameLen-2 {
		cs += uint32(byteOrder.Uint16(p[i : i+2]))
		if cs&0x80000000 > 0 {
			cs = (cs & 0xffff) + (cs >> 16)
		}
	}
