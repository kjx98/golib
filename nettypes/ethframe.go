...
var bigOrder = binary.BigEndian

func NToHS(aa []byte) uint16 {
	//_ = aa[1]
	//return uint16(aa[0])<<8 | uint16(aa[1])
	return bigOrder.Uint16(aa)
}

func NToHL(aa []byte) uint32 {
	//_ = aa[3]
	//return uint32(aa[0])<<24 | uint32(aa[1])<<16 | uint32(aa[2])<<8 | uint32(aa[3])
	return bigOrder.Uint32(aa)
}

func HToNS(v uint16) uint16 {
	b := (*[2]byte)(unsafe.Pointer(&v))
	return bigOrder.Uint16((*b)[:])
}

func HToNL(v uint32) uint32 {
	b := (*[4]byte)(unsafe.Pointer(&v))
	return bigOrder.Uint32((*b)[:])
}
