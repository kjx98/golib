...
func (t TCP_P) String(frameLen uint16, indent int, srcAddr, destAddr net.IP) string {
	return fmt.Sprintf(padLeft("TCP Len      : %d\n", "\t", indent), frameLen) +
		...
		fmt.Sprintf(padLeft("CalcChecksum : %02x\n", "\t", indent), HToNS(t.CalculateChecksum(frameLen, srcAddr, destAddr))) +
...
func (t TCP_P) WindowSize() uint16 {
	return NToHS(t[14:16])
}

func (t TCP_P) Checksum() uint16 {
	return NToHS(t[16:18])
}

func (t TCP_P) CalculateChecksum(frameLen uint16, srcAddr, destAddr net.IP) uint16 {
	cs := uint32(byteOrder.Uint16(t[0:2])) +
		uint32(byteOrder.Uint16(t[2:4])) +
		uint32(byteOrder.Uint16(t[4:6])) +
		uint32(byteOrder.Uint16(t[6:8])) +
		uint32(byteOrder.Uint16(t[8:10])) +
		uint32(byteOrder.Uint16(t[10:12])) +
		uint32(byteOrder.Uint16(t[12:14])) +
		uint32(byteOrder.Uint16(t[14:16])) +
		uint32(byteOrder.Uint16(t[18:20]))
	fl := frameLen - 20
	i := 20
	...
	cs += uint32(byteOrder.Uint16(srcAddr[0:2]))
	cs += uint32(byteOrder.Uint16(srcAddr[2:4]))
	cs += uint32(byteOrder.Uint16(destAddr[0:2]))
	cs += uint32(byteOrder.Uint16(destAddr[2:4]))
	cs += _PROTO_TCP
	cs += HToNL(uint32(frameLen))
	for cs>>16 > 0 {
		cs = (cs & 0xffff) + (cs >> 16)
	}
	return ^uint16(cs)
...
func (t TCP_P) SetChecksum(v uint16) {
	t[16] = byte(v >> 8)
	t[17] = byte(v & 0xff)
	//bigOrder.PutUint16(t[16:18], v)
}

func (t TCP_P) UrgPointer() uint16 {
	return NToHS(t[18:20])
}
