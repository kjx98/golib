...
var (
	errUnreachable = errors.New("Dest unreachable")
)
...
func getInt64(b []byte) (res int64) {
	_ = b[7]
...
func Ping(address string, count int) (cnt int, lat int64, err error) {
	var c net.Conn
	...
	dat := []byte("        Go go Ping!Ken sent me!!!")
	go func() {
		for i := 0; i < count; i++ {
			ii := time.Now().UnixNano()
			putInt64(dat, ii)
			wb, er := (&icmpMessage{
				Type: typ, Code: 0,
				Body: &icmpEcho{
	...
	for i := 0; i < count; {
		...
		case icmpv4EchoRequest, icmpv6EchoRequest:
			continue
		case icmpv4Unreachable, icmpv6Unreachable:
			err = errUnreachable
			fmt.Println("unreachanble")
			return
		case icmpv4EchoReply, icmpv6EchoReply:
			...
			if v, ok := m.Body.(*icmpEcho); ok {
				...
				tt := getInt64(v.Data)
				a := time.Now().UnixNano() - tt
				// a musr > 0 ns
				if a > 0 {

					lat += time.Now().UnixNano() - tt
				}
			}
		}
	}
